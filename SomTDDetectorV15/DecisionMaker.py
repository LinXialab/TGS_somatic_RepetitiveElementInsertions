...
def Decision(TDRecord, sequenceList, ReadIDs, flank_5,flank_3, Tlabel='tumor', readcutoff=3, hcutoff=3, scutoff=0.05):
    '''
    ...
    if (len(sequenceList) > 3) and (ReadTags.shape[0]>=2) and (np.min(Tagcount)>=3):
        ...
        if len(SomClust) > 0:
            ...
            if len([x for x in labels if x in SomClust]) >= readcutoff:
                seqencode_New, seqdatamx = MSAFeatureSelection(sequenceList, flank_5, flank_3, hcutoff=hcutoff, scutoff=scutoff)
