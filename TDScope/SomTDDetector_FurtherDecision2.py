...
def TDscope2(TDRecord, sequenceList, ReadIDs, flank_5,flank_3, data_queue, readcutoff=3, Tlabel="tumor"):
    '''
    ...
    try:
        ...
        if (seqdatamx.shape[0] != 0) and (seqdatamx.shape[1]>=10):
            ...
            ClusterAnno = {}
            somaticSeqCollect, germlineSeqCollect = [],[]
            somaticReadIDXCollect,germlineReadIDXCollect = [],[]
        ...
        data_queue.put([TDRecord, sequenceList, ReadIDs, flank_5,flank_3])
        return(None)

def TDScope_Complement(TDRecord, sequenceList, ReadIDs, flank_5,flank_3, readcutoff=3, Tlabel="tumor"):
    '''
    Complete pipeline of TDscope 
    :param:
        FileList: npz file list
    ...
    record = [chrom, start, end,
              ...
              "-",
              0,"-"]
    somaticSeqCollect, germlineSeqCollect,somaticReadIDXCollect, germlineReadIDXCollect= [],[],[],[]
    try:
        seqencode_New, seqdatamx = MSAFeatureSelection(sequenceList, flank_5, flank_3, hcutoff=3, scutoff=0.05)
        if (seqdatamx.shape[0] != 0) and (seqdatamx.shape[1]>=10):
            logging.info(f'{TDRecord} need EM clustering')
    ...
    with Manager() as manager:
        ...
        for npzFile in npzFileList:
            ...
            for I in range(Dat.shape[0]):
                ...
                result = P.apply_async(TDscope2, (TDRecord, sequenceList, ReadIDs, flank_5,flank_3, data_queue))
                results.append(result)
        # Set memory error collection
        f = open(os.path.join(args.savedir, rawoutput), 'w')
        outRecord = 0
        while results:
        ...
        while not data_queue.empty():
            TDRecord, sequenceList, ReadIDs, flank_5,flank_3 = data_queue.get()
            output = TDScope_Complement(TDRecord, sequenceList, ReadIDs, flank_5,flank_3)
            f.write("\t".join([str(x) for x in output]) + '\n')
            f.flush()
