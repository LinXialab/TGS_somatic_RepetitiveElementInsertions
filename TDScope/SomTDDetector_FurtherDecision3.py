...
Release Note:
    ...
    if aim region subseq outof memory, we will put the TD scope simple result to fill it, leave the last columns TDscope-simple flag
Requirement:
    os,re, argparse
...
def TDscope2(TDRecord, sequenceList, ReadIDs, flank_5,flank_3, readcutoff=3, Tlabel="tumor"):
    ...
    record = [chrom, start, end,
              ...
              "-",
              0,"-"]
    if (len([x for x in ReadIDs if re.search(Tlabel,x)]) < 3) or (len([x for x in ReadIDs if not re.search(Tlabel,x)]) < 3):
        record[-1] = 'No_enough_reads'
        return(record)
    else:
        try:
            ...
            if (seqdatamx.shape[0] != 0) and (seqdatamx.shape[1]>=10):
                ...
                for L in np.unique(DatLabel):
                    ...
                    if (ReadType.shape[0] == 1) and (ReadType[0]==Tlabel) and (ReadIDsub.shape[0]>=readcutoff):
                        ...
                        if ReadIDsub.shape[0] >= readcutoff:
            ...
            SomClust, GermClust, labels = FindSomClust(sequenceList[1:], ReadIDs, Tlabel)
            if len([x for x in labels if x in SomClust]) >= readcutoff :
                SupportReads = [",".join(list(ReadIDs[np.where(labels==x)])) for x in SomClust]
                record[4] = ";".join(SupportReads)
